                          ...
                          int flags)
{
    ...
    QTAILQ_FOREACH(wp, &cpu->watchpoints, entry) {
        if (addr == wp->_vaddr && len == wp->len /* AWH - Underscore */
...
void cpu_watchpoint_remove_by_ref(CPUState *cpu, CPUWatchpoint *watchpoint)
{
    ...
    tlb_flush_page(cpu, watchpoint->_vaddr); /* AWH - Underscore */
                                                  ...
                                                  vaddr len)
{
     ...
     * wraps round to zero.
     */
    vaddr wpend = wp->_vaddr + wp->len - 1; /* AWH - Underscore */
    vaddr addrend = addr + len - 1;

    return !(addr > wpend || wp->_vaddr > addrend); /* AWH - Underscore */
}
