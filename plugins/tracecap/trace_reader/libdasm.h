...
/* Moved these things into libdasm_part2.h"
// Registers
...
//LOK: Added const keyword in get_instruction_string
int get_instruction_string(
	const INSTRUCTION *inst,	// pointer to INSTRUCTION structure
...
//LOK: Not only added the const in front of the pointers, but also changed POPERAND to OPERAND*
int get_operand_string(
	const INSTRUCTION *inst,	// pointer to INSTRUCTION structure
	const OPERAND *op,		// pointer to OPERAND structure
        enum Format format,	// instruction format: FORMAT_ATT or FORMAT_INTEL
	DWORD offset,		// instruction absolute address
	char *string,		// string buffer
	int length		// string length
