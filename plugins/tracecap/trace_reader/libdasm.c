...
#if !defined NOSTR
int get_operand_string(const INSTRUCTION *inst, const OPERAND *op,
...
int get_instruction(PINSTRUCTION inst, BYTE *addr, enum Mode mode) {
	...
	//LOK: Moved it down, this definitely doesn't make sense up here
	// MODRM byte offset
	//if (ptr->modrm)
...
#if !defined NOSTR
int get_mnemonic_string(const INSTRUCTION *inst, enum Format format, char *string, int length) {
	int mode;
...
int get_operands_string(const INSTRUCTION *inst, enum Format format, DWORD offset,
...
//LOK::Added the const keyword in front of INSTRUCTION
int get_instruction_string(const INSTRUCTION *inst, enum Format format, DWORD offset,
		char *string, int length) {
